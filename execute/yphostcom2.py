...
def single_cmd_deal(i_obj_file,ttfname,date,sexComd,sexConf,sexParam1,psfexComd,psfexConf,scampComd, scampConf, scampConf2,sexComdPSF,sexParam2,rootpath,figdir,ancdir,tscidir,configdir):
    ...
    try:
        ...
        iobjFrameNew = i_obj_file
        iobjFrame_o0 = iobjFrameNew.split('subbkg_0')[0] + 'subbkg0.fits'
        ...
        fits.writeto(iobjFrame_o0,iimgMat,ihdr,overwrite=True)
        print(fits.getheader(iobjFrame_o0))
        ...
        # if os.path.exists( iobjFrame_o0[:-5] + "_sexcat.fits"):
        ...
        # try:
        #     os.system("mv *fits %s" % tscidir)  #这个存在报错mv: cannot stat ‘*pdf’: No such file or directory
        # except:
        #     logger.error('there is no any new fits move to scidir')
        ...
        if(os.path.exists(ihead)):
            ...
            ihdr["CUNIT2"] = "deg"
            fits.writeto(iobjFrame_o0, iimgMat, ihdr, overwrite=True)
        else :#(os.path.exists(ihead)==False):
        ...
        try:
            ...
            if len(idRef)==0:
                ...
                #astro=astrometry(ildacn,refCatn,iobjFrame_o0,scampComd, scampConf, scampConf2)
                # if astro is None:
                #     logger.error(f"os.path.exists(ihead)==False: {os.path.exists(ihead)==False}")
                #     logger.info(f"ending this loop")
                #     return None,None
        ...
        try:
            ...
            sex_comd(isexComd)
            #logger.info("[_mphot]:"+isexComd) #fy
            #os.system(isexComd)
        ...
        try:
            ...
            for i in range(0,len_ldac):
                if(float(ildac["FLUX_AUTO"][i])==0.0):
                    ildac["MAG_AUTO"][i] =  ildac["MAG_AUTO"][i]
                else:
                    ildac["MAG_AUTO"][i] = 1.0857 * (float(ildac["FLUXERR_AUTO"][i])/float(ildac["FLUX_AUTO"][i]))
                    
                if(float(ildac["FLUX_APER"][i][0])==0.0): 
                    ildac["MAG_APER"][i] =ildac["MAG_APER"][i]
                else:
                    ildac["MAG_APER"][i] = 1.0857 * (ildac["FLUXERR_APER"][i]/ildac["FLUX_APER"][i])
                try:
                    if(float(ildac["FLUX_PSF"][i])==0.0):
                        ildac["MAG_PSF"][i]=  ildac["MAG_PSF"][i]
                    else:
                        ildac["MAG_PSF"][i]= 1.0857 * (float(ildac["FLUXERR_PSF"][i])/float(ildac["FLUX_PSF"][i]))
                except Exception as e:
                        logger.error(str(traceback.format_exc()))
        ...
        # 1) region file
        mgfigname = figdir + "MagHist_%s.pdf" % (ildacn.split("/")[-1][:-5])
        ...
        hid = (iflag==0)
        imagAUTO_h   = imagAUTO[hid]
        # try:
        #     PlotFun.hist_mag(imagAUTO_h,figout=mgfigname)
        # except Exception as e:
        #         print(str(e))
        #         print('the astrometryPlot can not be generated!')     
        ...
        if len(idRef)==0:
            ...
            #astrometry(ildacn,refCatn,iobjFrame_o0,scampComd, scampConf, scampConf2)
        logger.info(f"[_mphot]: 2) determine the astrometric accuracy")
        ...
        logger.info("[_mphot]:"+" Astrometric scatter (%s stars): sigma_ra=%.3f, sigma_dec=%.3f"%(len(idRef),iraSig,idecSig))
        ifigname = figdir + "astrometry_%s.pdf" % (ildacn.split("/")[-1][:-5])
        cfigname = figdir + "astroCross_%s.pdf" % (ildacn.split("/")[-1][:-5])
        try:
            #PlotFun.astrometryPlot(idra, iddec, sigma=(iraSig, idecSig), figout=ifigname)
            #PlotFun.astrometry_crossmatch(idra, iddec, idximg, idyimg, binfact, figout=cfigname)
            logger.info("[_mphot]:"+'plotok')
        except Exception as e:
                logger.error(str(traceback.format_exc()))
                logger.info(' Astrometric scatter generating failed!')
        

        logger.info(f"[_mphot]: 3) determine the zeropoint")
        ...
        # bkgNoise = np.sqrt(np.pi) * np.median(ifwhm) * (
        #     np.min([ihdr["SKYRMS1"], ihdr["SKYRMS2"], ihdr["SKYRMS3"], ihdr["SKYRMS4"]]))
        ...
        try:
            ...
            logger.info(f"[_mphot]: 4) show the Whisker plot")
        except Exception as e:
            logger.error(str(traceback.format_exc()))
            logger.info(' Whisker plot generating failed!')
 
        try:
            ldacdata = fits.open(ildacn)
            ldacdata.writeto(iobjFrame_o0[:-5] + "_sexcat.fits", overwrite=True)
            filename = iobjFrame_o0.rsplit('/', 1)[1]
            ldacdata.close() #open之后记得关闭
            logger.info("[_mphot]:"+filename + ' has been successfully processed!')
        except Exception as e:
            logger.error(str(traceback.format_exc()))
            logger.info("[_mphot]:"+filename +' generating failed!')
        
        
        
    except:
        logger.error(str(traceback.format_exc()))
        logger.info("[_mphot]:"+i_obj_file + ' has been fail processed!')
    finally:
...
def psf_comd(ipsfComd):
    ...
    try:
        # 执行命令并设置超时时间为10秒
        completed_process_ipsfcomd = subprocess.run(ipsfComdlist, timeout=300, shell=False, capture_output=True)
...
def photometry(rootpath,ttfname,date):
    ...
    if not(os.path.exists(scipath+'fileguide/'+ttfname+'_subbkg_o0.npy')):
    ...
    tid=ttf[0]
    targetid=ttf[1]
    if len(lpathsubbkg)==0:
        logger.error (f"lpathsubbkg:{lpathsubbkg}")
        return None
    
    ira='0'
    idec='0'
    #if os.path.exists(lpathsubbkg[0]):
    ...
    for i in range(0,len(lpathsubbkg)):
            try:
                ...
                logger.error ("Error: 没有找到文件或读取文件失败")
                continue
    ...
    scampComd2 = " -FWHM_THRESHOLDS 2,20 -SN_THRESHOLDS 15,1000"
    ...
    for i,file_name in enumerate(lpathsubbkg):
        ...
        iobjFrame_o0=iobjFrameNew.split('subbkg')[0]+date+'subbkg.fits'
        ipsfFrameNew = iobjFrame_o0[:-4] + "psf"
        try:
            ihdr,iimgMat= readfits(iobjFrameNew)
            r,c=np.shape(iimgMat)
        ...
        # imgnew,hdrnew=fits.getdata(iobjFrameNew,header=True)
        # hdrnew["CTYPE1"]  = "RA---TAN"
        # hdrnew["CTYPE2"]  = "DEC--TAN"
        # hdrnew["EQUINOX"] = 2000.0
        # hdrnew["RADESYS"] = "ICRS"
        # hdrnew["CUNIT1"]  = "deg"
        # hdrnew["CUNIT2"]  = "deg"
        # hdrnew["CRVAL1"]  = ira
        ...
        # hdrnew["CRPIX1"]  = c/2
        # hdrnew["CRPIX2"]  = r/2
        # hdrnew["CD1_1"]   = -pscale/3600.0
        # hdrnew["CD1_2"]   = 0.0
        # hdrnew["CD2_1"]   = 0.0
        # hdrnew["CD2_2"]   = -pscale/3600.0
        # fits.writeto(iobjFrameNew,imgnew,hdrnew,overwrite=True)
