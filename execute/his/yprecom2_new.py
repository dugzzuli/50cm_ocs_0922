...
def combine_process(filename,ipath,calpath,opath,libpath,date):
    ...
    try:
        ...
        hdr_subbkg['GAIN']=((gain1+gain2)/2)*exptime
        hdr_subbkg['BUNIT']='ADU/s'
    ...
    try:
        ...
        flat_corrected2=(img2-master_bias2)/master_flat2
            ...
            try:
                print(np.shape(img1))
                img1=img[:,500:int(c/2)]
                img2=img[:,int(c/2):int(c)-500] 
                print(np.shape(flat_corrected1))
                flat_corrected1=(img1-master_bias1)/master_flat1
                flat_corrected2=(img2-master_bias2)/master_flat2
    ...
    flat_corrected1_s=flat_corrected1/exptime
    
    flat_corrected2_s=flat_corrected2/exptime
    ...
    #写入fits头
    saturate1=(65535.0-sigma_clipped_stats(master_bias1)[1])/exptime-np.nanmedian(bkg1)
    saturate2=(65535.0-sigma_clipped_stats(master_bias2)[1])/exptime-np.nanmedian(bkg2)
    saturate1=round(saturate1,2)
    saturate2=round(saturate2,2) 
    hdr_subbkg["SATURAT1"] = saturate1
    hdr_subbkg["SATURAT2"] = saturate2
    hdr_subbkg["SATURATE"] = np.min([saturate1,saturate2])
