...
def photometry(rootpath,ttfname,date):
    ...
    for i,file_name in enumerate(lpathsubbkg):
        ...
        return_value=os.system(iscampComd)
        ihead   = ildacn[:-4] + "head"
        ...
        if(return_value==255):
            logger.error(f"the scamp is calling failed")
        ihead   = ildacn[:-4] + "head"
       ...
       # generate the final catalog
        ...
        for i in range(0,len_ldac):
            try:
                ildac["MAG_AUTOERR"][i] = 1.0857 * (ildac["FLUXERR_AUTO"][i]/ildac["FLUX_AUTO"][i])
            except Exception as e:    
                ildac["MAG_AUTOERR"][i] =  ildac["MAGERR_AUTO"][i]
            try:
                ildac["MAGERR_APER"][i] = 1.0857 * (ildac["FLUXERR_APER"][i]/ildac["FLUX_APER"][i])
            except Exception as e:    
                ildac["MAGERR_APER"][i]=ildac["MAG_APER"][i]
            try:
                ildac["MAGERR_PSF"][i]= 1.0857 * (ildac["FLUXERR_PSF"][i]/ildac["FLUX_PSF"][i])
            except Exception as e:
                ildac["MAGERR_PSF"][i]=  ildac["MAGERR_PSF"][i]
        ##fy 20230217#####
